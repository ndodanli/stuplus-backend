...
<script>
import { fetchList, fetchPv, addUpdateAnnouncementLike, deleteAnnouncementLike } from '@/api/announcementLike'
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addUpdateAnnouncementLike(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addUpdateAnnouncementLike(tempData).then(() => {
    ...
    handleDelete(row, index) {
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          deleteAnnouncementLike({ _id: row._id }).then(() => {
            this.$notify({
              title: 'Success',
