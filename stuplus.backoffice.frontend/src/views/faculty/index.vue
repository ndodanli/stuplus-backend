      ...
      <el-table-column label="School" prop="schoolId" align="center">
        <template slot-scope="{ row }">
          <span style="font-size:14px;" class="m-1">
            <el-tag type="success"> {{ schools.find(school => school._id ===
                row.schoolId)?.title
            }}
            </el-tag>
          </span>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left"
        ...
        <el-form-item label="School" prop="schoolId">
          <el-select v-model="temp.schoolId" filterable placeholder="Select school..." remote reserve-keyword
            :remote-method="remoteMethod" :loading="remoteLoading">
            <el-option v-for="item in searchedSchools" :key="item._id" :label="item.title" :value="item._id">
            </el-option>
          </el-select>
        </el-form-item>
...
<script>
import { fetchList, fetchPv, addUpdateFaculty, deleteFaculty } from '@/api/faculty'
import { getSchools } from '@/api/general'
...
export default {
  ...
  data() {
    return {
      token: getToken(),
      uploadFilePath:
        'process.env.VUE_APP_BASE_API/general/uploadFile?uploadPath=faculty_images',
      tableKey: 0,
      list: null,
      total: 0,
      listLoading: true,
      ...
      rules: {
        ...
        schoolId: [
          { required: true, message: 'School is required', trigger: 'blur' }
        ]
      },
  ...
  methods: {
    async remoteMethod(query) {
      if (query !== '') {
        ...
        const schoolResult = await getSchools({ search: query });
        this.searchedSchools = schoolResult?.data
        this.schools = this.searchedSchools.concat(this.schools)
    ...
    getList() {
      ...
      fetchList(this.listQuery).then((response) => {
        ...
        this.searchedSchools = response.data.schools;
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addUpdateFaculty(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addUpdateFaculty(tempData).then(() => {
    ...
    handleDelete(row, index) {
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          deleteFaculty({ _id: row._id }).then(() => {
            this.$notify({
              title: 'Success',
