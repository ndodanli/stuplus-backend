    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left">
        ...
        <el-form-item label="Question" prop="questionId">
          <el-select v-model="temp.questionId" filterable placeholder="Select question..." remote reserve-keyword
            :remote-method="remoteMethodQuestion" :loading="remoteLoading">
...
<script>
import { fetchList, fetchPv, addUpdateQuestionComment, deleteQuestionComment } from '@/api/questionComment'
import waves from '@/directive/waves' // waves directive
import { formatDate, parseTime } from '@/utils'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination
import { getToken } from '@/utils/auth'
import { getUsers, getQuestions } from '@/api/general'
...
export default {
  ...
  methods: {
    ...
    async remoteMethodQuestion(query) {
      if (query !== '') {
        this.remoteLoading = true;
        const questionResult = await getQuestions({ search: query });
        this.searchedQuestions = questionResult?.data
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addUpdateQuestionComment(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addUpdateQuestionComment(tempData).then(() => {
    ...
    handleDelete(row, index) {
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          deleteQuestionComment({ _id: row._id }).then(() => {
            this.$notify({
              title: 'Success',
