<template>
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"
      ...
      <el-table-column label="Question" prop="questionId" align="center">
        <template slot-scope="{ row }">
          <span style="font-size:14px;" class="m-1">
            <el-tag type="success"> {{ questions.find(question => question._id ===
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left">
        ...
        <el-form-item label="Question" prop="questionId">
          <el-select @change="handleQuestionChange(temp.questionId)" v-model="temp.questionId" filterable
            placeholder="Select question..." remote reserve-keyword :remote-method="remoteMethodQuestion"
            :loading="remoteLoading">
            <el-option v-for="item in searchedQuestions" :key="item._id" :label="item.title" :value="item._id">
            </el-option>
          </el-select>
        </el-form-item>
...
<script>
import { fetchList, fetchPv, addUpdateQuestionCommentLike, deleteQuestionCommentLike } from '@/api/questionCommentLike'
import waves from '@/directive/waves' // waves directive
import { formatDate, parseTime } from '@/utils'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination
import { getToken } from '@/utils/auth'
import { getUsers, getQuestions, getQuestionComments } from '@/api/general'
...
export default {
  ...
  methods: {
    async handleQuestionChange(questionId) {
      const commentsResult = await getQuestionComments({ questionId: questionId });
    ...
    async remoteMethodQuestion(query) {
      if (query !== '') {
        this.remoteLoading = true;
        const annoResult = await getQuestions({ search: query });
        this.searchedQuestions = annoResult?.data
        this.questions = this.searchedQuestions.concat(this.questions)
        this.remoteLoading = false;
      } else {
        this.questions = [];
      }
    ...
    getList() {
      ...
      fetchList(this.listQuery).then((response) => {
        ...
        this.searchedQuestions = response.data.questions;
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addUpdateQuestionCommentLike(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addUpdateQuestionCommentLike(tempData).then(() => {
    ...
    handleDelete(row, index) {
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          deleteQuestionCommentLike({ _id: row._id }).then(() => {
            this.$notify({
              title: 'Success',
