...
<script>
import { fetchList, addUpdateQuestion, deleteQuestion } from '@/api/question'
...
export default {
  ...
  data() {
    return {
      ...
      token: getToken(),
      uploadFilePath:
        'process.env.VUE_APP_BASE_API/general/uploadFile?uploadPath=question/cover_images',
      tableKey: 0,
      list: null,
      total: 0,
      listLoading: true,
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addUpdateQuestion(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addUpdateQuestion(tempData).then(() => {
    ...
    handleDelete(row, index) {
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          deleteQuestion({ _id: row._id }).then(() => {
            this.$notify({
              title: 'Success',
