...
router.post("/", validateLogin, async (req: CustomRequest<LoginUserDTO>, res: any) => {
    ...
    try {
        response.data = await UserAccess.loginUser(req.acceptsLanguages(), new LoginUserDTO(req.body));
...
router.post("/register", validateRegister, async (req: CustomRequest<RegisterUserDTO>, res: any) => {
    const response = new BaseResponse<object>();
    try {
        response.data = await UserAccess.registerUser(req.acceptsLanguages(), new RegisterUserDTO(req.body))

        response.setMessage(getMessage("accountCreated", req.acceptsLanguages()))
